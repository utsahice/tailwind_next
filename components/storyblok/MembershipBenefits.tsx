"use client";
import { storyblokEditable } from "@storyblok/react/rsc";

interface Benefit {
    _uid: string;
    icon: string;
    title: string;
    description: string;
}

interface MembershipBenefitsProps {
    blok: {
        title: string;
        benefits: Benefit[];
        _uid: string;
        component: string;
    };
}

const iconMap: { [key: string]: React.ReactElement } = {
    star: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
        </svg>
    ),
    discount: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
    ),
    community: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
    ),
    support: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
    ),
};

export default function MembershipBenefits({ blok }: MembershipBenefitsProps) {
    return (
        <section {...storyblokEditable(blok)} className="py-20 bg-gray-50">
            <div className="max-w-[1800px] mx-auto px-6 md:px-12 lg:px-[100px]">
                <h2 className="font-ivy text-5xl md:text-6xl text-center text-black mb-16">
                    {blok.title || "Member Benefits"}
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {blok.benefits?.map((benefit) => (
                        <div
                            key={benefit._uid}
                            className="bg-white rounded-xl p-8 text-center hover:shadow-xl transition-shadow duration-300"
                        >
                            <div className="inline-flex items-center justify-center w-20 h-20 bg-yello/10 rounded-full mb-6 text-yello">
                                {iconMap[benefit.icon] || iconMap.star}
                            </div>
                            <h3 className="font-ivy text-2xl text-black mb-4">
                                {benefit.title}
                            </h3>
                            <p className="font-termina text-sm text-gray-600">
                                {benefit.description}
                            </p>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    );
}
