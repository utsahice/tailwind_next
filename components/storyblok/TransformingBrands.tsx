...
interface TransformingBrandsProps {
    blok: {
        title?: string;
        subtitle?: string;
        description?: string;
...
export default function TransformingBrands({ blok }: TransformingBrandsProps) {
    // Fallback statistics from original design
    const fallbackStats = [
    ...
    const stats = blok.statistics?.length ? blok.statistics.map(stat => ({
        title: stat.label.split(' ')[0] || "EXPERIENCE",
        value: stat.number,
        desc: stat.label
    })) : fallbackStats;
    ...
    return (
        <section {...storyblokEditable(blok)} className="py-16 lg:py-24 bg-white">
            <h2 className="text-center text-black font-ivy text-3xl md:text-4xl lg:text-5xl mb-10 tracking-wide">
                {blok.title || "TRUSTED BY GLOBAL BRAND EXPERTS"}
            </h2>
            ...
            <div className="max-w-9xl bg-gray mx-auto px-6 flex flex-col lg:flex-row gap-12">
                <div className="w-full lg:w-auto flex-shrink-0">
                    <div className="mx-auto lg:mx-0 h-[650] max-w-[280px] rounded-lg overflow-hidden shadow-lg transform lg:-translate-y-10">
                        <Image
                            src={blok.background_image?.filename || "/transforming/1.jpg"}
                            alt={blok.background_image?.alt || "Transforming brands"}
                            width={400}
                            height={800}
                ...
                <div className="w-full lg:w-3/4 overflow-x-auto ml-0.5">
                    ...
                    <h2 className="text-2xl lg:text-3xl font-ivy font-semibold text-yello mb-8 whitespace-nowrap">
                        {blok.subtitle || "Transforming Brands Into Global Icons"}
                    </h2>
