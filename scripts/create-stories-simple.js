...
async function createStoriesSimple() {
  try {
    console.log('🚀 Creating Stories with Simple API Calls...\n');
    ...
    try {
      ...
      const navbarResponse = await Storyblok.post('cdn/stories/', {
        story: navbarStory,
        publish: 1
      });
      console.log(`✅ Global Navbar created successfully`);
    } catch (error) {
      console.log('❌ Error creating navbar:', error.response?.status, error.response?.data?.error || error.message);
    }
    ...
    try {
      ...
      const footerResponse = await Storyblok.post('cdn/stories/', {
        story: footerStory,
        publish: 1
      });
      console.log(`✅ Global Footer created successfully`);
    } catch (error) {
      console.log('❌ Error creating footer:', error.response?.status, error.response?.data?.error || error.message);
    }
    ...
    try {
        ...
        content: {
          ...
          body: [
              ...
              values: [
                {
                  _uid: 'value_1',
                  title: 'Excellence',
                  description: 'We deliver exceptional results that exceed expectations, setting new standards in beauty brand development.',
                  icon: { filename: '/service_1.jpg', alt: 'Excellence' }
                },
                {
                  _uid: 'value_2',
                  title: 'Innovation',
                  description: 'We embrace cutting-edge technologies and creative approaches to solve complex brand challenges.',
            ...
            // Team Section
            {
              ...
              description: 'Our leadership team brings decades of combined experience from the world\'s most prestigious beauty brands.',
              team_members: [
                {
                  _uid: 'team_1',
                  name: 'Sarah Johnson',
            ...
            // Company History Section
            {
              ...
              description: 'From our humble beginnings in Beverly Hills to becoming a global leader in beauty brand development.',
              milestones: [
                {
                  _uid: 'milestone_1',
                  year: '1998',
      ...
      const aboutResponse = await Storyblok.post('cdn/stories/', {
        story: aboutStory,
        publish: 1
      });
      console.log(`✅ About Us page created successfully`);
    } catch (error) {
      console.log('❌ Error creating about page:', error.response?.status, error.response?.data?.error || error.message);
    }

    console.log('\n🎉 Story Creation Process Complete!\n');
    ...
    console.log('   2. Look for the new stories in the content section');
    console.log('   3. If stories exist, they should now have content');
    console.log('   4. If not created, we may need to use the Storyblok UI');
    console.log('');
    console.log('📋 ALTERNATIVE APPROACH:');
    console.log('   If stories are not showing, you can:');
    console.log('   1. Create stories manually in Storyblok UI');
    console.log('   2. Use the component names: Navbar, Footer, page');
    console.log('   3. Copy content from the fallback data in Layout.tsx');
    console.log('');
    console.log('✨ The components are ready - just need the stories in Storyblok!');

  } catch (error) {
    console.error('❌ Error in create stories:', error.response?.data || error.message);
  }
}
