...
async function fixUndefinedImages() {
  try {
    ...
    const getAssetUrl = (filename, fallback) => {
      if (!filename || filename === 'undefined') return fallback;
      return assets[filename] || fallback;
    };

    // Create story content with fixed image URLs
    const storyContent = {
      component: "Page",
      body: [
        // Hero Section
    ...
    // Update the story
    console.log('\n🔄 Updating home story with fixed image URLs...');
    ...
    // Log sections added
    console.log('\n📋 All body blocks updated with fixed image URLs:');
